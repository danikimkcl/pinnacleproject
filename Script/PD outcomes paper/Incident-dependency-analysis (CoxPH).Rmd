...
res.cox = survival::coxph(Surv(censor_days/2, dependency) ~ parkinsonism, data = data)
test.ph = survival::cox.zph(res.cox)
test.ph

res.cox = survival::coxph(Surv(censor_days/2, dependency) ~ parkinsonism + age + gender + illnocat + PEDUC, data = data)
test.ph = survival::cox.zph(res.cox)
test.ph
```
...
hrs_selfpd_care_crude= func_coxph(data=data, exposure="parkinsonism", adj="")

hrs_pnism_care_adj = func_coxph(data=data, exposure="pnism", adj=" + age + gender + PEDUC + illnocat")
hrs_pd_care_adj = func_coxph(data=data, exposure="JorgePD", adj=" + age + gender + PEDUC + illnocat")
hrs_selfpd_care_adj = func_coxph(data=data, exposure="parkinsonism", adj=" + age + gender + PEDUC + illnocat")
```
...
### Self-reported PD and incident dependency (crude & adjusted for age, sex, education, and illnocat)

```{r, fig.width = 10, fig.height = 6}
plot_selfpd_care_crude = plot_forest_meta(hrs_selfpd_care_crude[-1, ])
plot_selfpd_care_adj = plot_forest_meta(hrs_selfpd_care_adj[-1, ])
```

## Combine and save outputs

```{r}
plot_combined_care_adj = cowplot::plot_grid(plot_pnism_care_adj, plot_selfpd_care_adj, plot_pd_care_adj,
                                         align = "hv",
                                         labels = "AUTO", 
                                         scale = 0.5,
                                         ncol = 1)

ggsave(plot = plot_combined_care_adj, filename = here::here("Output", "PD outcomes paper", "plots", "hrs_IncidentDependency_adj1.png"), width = 22, height = 20, units = "cm") # width = 22, height = 32 for 3 rows

plot_combined_care_crude = cowplot::plot_grid(plot_pnism_care_crude, plot_selfpd_care_crude, plot_pd_care_crude,
                                         align = "hv",
                                         labels = "AUTO", 
                                         scale = 0.5,
                                         ncol = 1)

ggsave(plot = plot_combined_care_crude, filename = here::here("Output", "PD outcomes paper", "plots", "hrs_IncidentDependency_crude1.png"), width = 22, height = 20, units = "cm") # width = 22, height = 32 for 3 rows
```
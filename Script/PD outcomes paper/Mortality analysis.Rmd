...
```{r}
data2 = data %>% 
  select(age, countryid, rural, gender, PMARRY, PEDUC, CARENEED, PSMOKE, PACTIVE, Nassets, illnocat, parkinsonism, JorgePD_or_selfPD, died, censor_days) %>% 
  sample_n(500)
```
...
```{r}
imp = mice::mice(data2, m=5, maxit=10, printFlag=FALSE, seed=100)
...
```{r}
fit = with(data=imp, exp=coxph(Surv(censor_days, died) ~ JorgePD_or_selfPD + age + gender + strata(illnocat) + PEDUC))

pooled = pool(fit)

pooled_stat = summary(pooled, conf.int = TRUE) %>% 
  select(-statistic, -df)

cbind(term=pooled_stat[, "term"], exp(pooled_stat[, c("estimate", "2.5 %", "97.5 %")]), pval=pooled_stat[, "p.value"])
```
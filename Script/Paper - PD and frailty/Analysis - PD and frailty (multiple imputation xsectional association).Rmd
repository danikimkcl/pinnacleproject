...
Multiple imputation analysis of the cross-sectional associations between parkinsonism and PD with frailty (defined according to the frailty phenotype, multidimensional frailty, and frailty index) using the baseline 10/66 survey.
...
path = here::here("Output", "PD and frailty paper", "imputed_data.rds")
saveRDS(impData, file=path)
```

Open imputed data - no longer need to repeat above multiple imputation step!
...
```{r}
all.impdata2 = all.impdata[!all.impdata$.imp==0, ]
```

## Analysis of multiply imputed datasets

### Count of frailty at baseline

```{r}
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(summary(x$b_fp.frail),
                                                                  summary(x$b_mf.frail),
                                                                  summary(x$b_fi.frail))))
pool_prev

Reduce("+",pool_prev)/length(pool_prev)
```

### Venn diagram of frail participants at baseline for each imputed dataset

```{r}
venn_diagram = function(df, i){
  x = list(`FI` = df %>% filter(.imp==i) %>% filter(b_fi.frail==1) %>% pull(.id), 
           `FP` = df %>% filter(.imp==i) %>% filter(b_fp.frail==1) %>% pull(.id), 
           `MF` = df %>% filter(.imp==i) %>% filter(b_mf.frail==1) %>% pull(.id))

  p = ggVennDiagram::ggVennDiagram(x, 
...
venn_diagram(all.impdata2, 1)
venn_diagram(all.impdata2, 2)
venn_diagram(all.impdata2, 3)
venn_diagram(all.impdata2, 4)
venn_diagram(all.impdata2, 5)
```

### Prevalence of frailty by PD/parkinsonism

```{r}
# Function to process output 
process_by_output = function(by_object){
  
  pool_prev1 = data.frame(do.call("cbind", by_object)) %>% 
    mutate(exp = c("no", "yes", "no", "yes"),
           frail = c("no", "no", "yes", "yes")) %>% 
    pivot_longer(X1:X5, names_to="x") %>% 
    arrange(x)
  ...
  pool_prev1 = pool_prev1 %>% 
    left_join(temp, by = c("x", "exp")) %>% 
    mutate(pct = format(round(value/tot*100, 1), nsmall=1),
           pct2 = str_replace_all(paste0(" (", pct, "%)"), " ", ""),
           prev = paste0(value, " ", pct2)) %>% 
    select(x, exp, frail, prev) %>% 
    pivot_wider(names_from = frail, values_from = prev) 

  colnames(pool_prev1) = c("x", "exp", "frail_no", "frail_yes")
...
```{r}
# Pnism and FP
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$pnism, x$b_fp.frail)))) # compute cross-tabulation of pnism/pd and frailty for each imputed dataset
pool_prev_pnism_fp = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets

# Pnism and MF
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$pnism, x$b_mf.frail))))
pool_prev_pnism_mf = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets

# Pnism and FI
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$pnism, x$b_fi.frail))))
pool_prev_pnism_fi = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets

# PD and FP
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$PD, x$b_fp.frail))))
pool_prev_pd_fp = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets

# PD and MF
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$PD, x$b_mf.frail))))
pool_prev_pd_mf = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets
 
# PD and FI
pool_prev = with(all.impdata2, by(all.impdata2, .imp, function(x) c(table(x$PD, x$b_fi.frail))))
pool_prev_pd_fi = process_by_output(pool_prev)
Reduce("+",pool_prev)/length(pool_prev) # calculate average of imputed datasets
```
...
```{r}
save_pool_prev_pnism_frail = pool_prev_pnism_fp %>% 
  left_join(pool_prev_pnism_mf, by = c("x", "exp")) %>% 
  left_join(pool_prev_pnism_fi, by = c("x", "exp"))

colnames(save_pool_prev_pnism_frail) = c("imp", "parkinsonism", "fp_no", "fp_yes", "mf_no", "mf_yes", "fi_no", "fi_yes")

save_pool_prev_pd_frail = pool_prev_pd_fp %>% 
  left_join(pool_prev_pd_mf, by = c("x", "exp")) %>% 
  left_join(pool_prev_pd_fi, by = c("x", "exp"))

colnames(save_pool_prev_pd_frail) = c("imp", "pd", "fp_no", "fp_yes", "mf_no", "mf_yes", "fi_no", "fi_yes")

write.csv(save_pool_prev_pnism_frail, file = here::here("Output", "PD and frailty paper", "Table_mi_prev_pnism_frail.csv"))
write.csv(save_pool_prev_pd_frail, file = here::here("Output", "PD and frailty paper", "Table_mi_prev_pd_frail.csv"))
```

...
Compute n and N from data (skip step as already did manual calculation)

```{r, include=FALSE}
# Process new variables
data = data %>% 
  separate(pct_pd, into = c("prop_pd", "prop_ci_pd"), sep = "[()]", extra = "merge") %>% 
  separate(prev_pd, into = c("prev_pd", "prev_ci_pd"), sep = "[()]", extra = "merge") %>% 
  separate(pct_pnism, into = c("prop_pnism", "prop_ci_pnism"), sep = "[()]", extra = "merge") %>% 
  separate(prev_pnism, into = c("prev_pnism", "prev_ci_pnism"), sep = "[()]", extra = "merge") 

### PD
x = subset(data_tot, (is.na(data_tot[, "N_pd"]) & is.na(data_tot[, "prop_pd"]) & !is.na(data_tot[, "prev_pd"]))) 
x$N_pd = (x$n_pd*100000)/x$prev_pd

y = subset(data_tot, (is.na(data_tot[, "N_pd"]) & !is.na(data_tot[, "prop_pd"]) & is.na(data_tot[, "prev_pd"]))) 
y$N_pd = (y$n_pd*100)/y$prop_pd

z = subset(data_tot, (is.na(data_tot[, "n_pd"]) & is.na(data_tot[, "prop_pd"]) & !is.na(data_tot[, "prev_pd"]))) 
z$n_pd = (z$N_pd*z$prev_pd)/100000

w = subset(data_tot, (is.na(data_tot[, "n_pd"]) & !is.na(data_tot[, "prop_pd"]) & is.na(data_tot[, "prev_pd"]))) 
w$n_pd = (w$N_pd*w$prop_pd)/100

## Combine results
results_pd = bind_rows(x, y, z, w, data_tot) %>% 
  arrange(studyid) %>% 
  group_by(study, country, urbanrural) %>% 
  mutate(rowid = row_number()) %>% 
  filter(rowid==1) %>% 
  select(-ends_with("pnism"))
```
...
```{r}
m.gen.reg = meta::metareg(meta_pd, ~min)
```

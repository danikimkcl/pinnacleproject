...
```{r}
varnames = data %>% 
  select(houseid2, PARTICID, countryid, centreid, age, gender, rural, Nassets, PMARRY, CARENEED, illnocat, cdem1066, PEDUC, PTOLDDM, PSMOKE, PALCNOW, PALCPAST, PACTIVE, PMEATFRQ, PFISHFRQ, PVEGS, NEO14G, NEO6AU, NEO6AL, NEO6BU, NEO6BL, NEO4E, NEO4F, NOE1, NEO14E, PCVA, medserv, PHOSAD, pnism, JorgePD, parkinsonism) %>% 
  names()
...
### Self-reported PD and mortality

```{r}
descr::crosstab(data$died, data$parkinsonism, prop.c=TRUE,  chisq=TRUE)
```
...
hrs_selfpd_died_crude = func_coxph(data=data, exposure="parkinsonism", adj="")
hrs_selfpd_died_adj = func_coxph(data=data, exposure="parkinsonism", adj=" + age + gender + PEDUC + strata(illnocat)")
```

### Parkinsonism and all-cause mortality (Adjusted for age and sex, and stratified by no. illnesses)

```{r}
kable(hrs_pnism_died_adj[-1, 1:4])
```

### PD and all-cause mortality (Adjusted for age and sex, and stratified by no. illnesses)

```{r}
kable(hrs_pd_died_adj[-1, 1:4])
```

### Self-reported PD and mortality (Adjusted for age and sex, and stratified by no. illnesses)

```{r}
kable(hrs_selfpd_died_adj[-1, 1:4])
```
...
#### Adjusted for age and sex, PEDUC, and stratified by no. illnesses
...
### Self-reported PD and all-cause mortality 
...
mgen = meta::metagen(TE = estimate,
                     ...
                     hakn = TRUE,                             # Hartung-Knapp adjustment of SEs
                     prediction = FALSE,                       # the above line and this line allow more uncertainty in the estimation of between-study heterogeneity and appropriate for use in random-effects meta-analysis
                     data = hrs_selfpd_died_crude[-1, ],
                     title = "Association between Self-reported PD and all-cause mortality")

meta::forest(mgen, 
             rightlabs = c("HR", "95% CI", "Weight"),
             print.tau2 = FALSE,
             leftcols = c("studlab"))
plot_selfpd_died_crude = grid.grab()
```
...
meta::forest(mgen, 
             ...
             print.tau2 = FALSE,
             leftcols = c("studlab"))
plot_selfpd_died_adj = grid.grab()
```

## Combine and save outputs

```{r}
plot_combined_crude = cowplot::plot_grid(plot_pnism_died_crude,  plot_pd_died_crude,
                                         align = "v",
                                         labels = "AUTO", 
                                         scale = 0.5,
                                         ncol = 1)
...
ggsave(plot = plot_combined_crude, filename = here::here("Output", "PD outcomes paper", "plots", "hrs_mortality_crude.png"), width = 18, height = 15, units = "cm") # height = 32 for 3 rows; height = 20 for 2 rows

plot_combined_adj = cowplot::plot_grid(plot_pnism_died_adj, plot_pd_died_adj, 
                                         align = "v",
                                         labels = "AUTO", 
                                         scale = 0.5,
                                         ncol = 1)

...
save_table1 = print(table1, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
save_table1_tot = print(table1_tot, quote = FALSE, noSpaces = TRUE, printToggle = FALSE)
...
ggsave(here::here("Output", "Neuropsychiatric symptoms in PD paper", "npi_prev_group.png"), plot = plot_npi_prev_grouped, width = 24, height = 12, units = "cm")
```
...
```{r}
clinical_sig = mydata %>% 
  select(NPI1D, starts_with("NPI"), -ends_with("SEV"), combined, -npidis) %>% 
  # only include participants who exhibit symptoms (i.e. as denominator)
  ...
  select(combined, starts_with("NPI"), -ends_with("DIS"), -NPI1D) 
